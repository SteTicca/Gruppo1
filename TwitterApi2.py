...
endpoint = "tweets/search/30day/ProgettoG1.json"

SEARCH_TERM = '#immigrazione OR #immigrati OR #migrazione OR #migranti'


def get_all_tweets(query):
    alltweets = []
    new_tweets = twitter.request('tweets/search/30day/:ProgettoG1', {'query': query, 'maxResults': 100,'fromDate':201806100000, "toDate":201806300000})
    next = json.loads(new_tweets.text)["next"]
    while (tweet["created_at"] != "Sun Jun 11 00:00:00 +0000 2018" for tweet in new_tweets):
        new_tweets = twitter.request('tweets/search/30day/:ProgettoG1', {'query': query, 'maxResults':100, 'fromDate':201806100000, "toDate":201806300000, "next":next})
